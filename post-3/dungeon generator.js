...
function create ()
{
    ...
    groundLayer = map.createBlankDynamicLayer('Layer 1', tileset);

    groundLayer.setScale(3);
    groundLayer.fill(20); // Fill with black tiles
    ...
    dungeon.rooms.forEach(function (room) {
        ...
        if (rand <= 0.25)
        {
            groundLayer.putTileAt(166, cx, cy); // Chest
        }
        else if (rand <= 0.3)
        {
            groundLayer.putTileAt(81, cx, cy); // Stairs
        }
        else if (rand <= 0.4)
        {
            groundLayer.putTileAt(167, cx, cy); // Trap door
        }
        else if (rand <= 0.6)
        {
            if (room.height >= 9)
    ...
    // do for the example.
    groundLayer.setCollisionByExclusion([ 6, 7, 8, 26 ]);

    // Hide all the rooms
    groundLayer.forEachTile(function (tile) { tile.alpha = 0; });

    // Place the player in the first room
    var playerRoom = dungeon.rooms[0];
    player = this.add.graphics({ fillStyle: { color: 0xedca40, alpha: 1 } })
        .fillRect(0, 0, map.tileWidth * groundLayer.scaleX, map.tileHeight * groundLayer.scaleY);
    ...
    cam.setBounds(0, 0, groundLayer.width * groundLayer.scaleX, groundLayer.height * groundLayer.scaleY);
...
{
    var tw = map.tileWidth * groundLayer.scaleX;
    var th = map.tileHeight * groundLayer.scaleY;
